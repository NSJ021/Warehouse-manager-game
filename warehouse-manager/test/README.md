...
| `integration/` | Two processes: grab, two-player carry, handoff, release, **solo drag and its promotion to a carry**, then a second scenario for **storage** -- place, refuse, place, retrieve, LIFO -- then a third for **a day** -- `begin_run()`, a broadcast manifest, the truck dump, the door, and who may call it a night -- all agreeing | ~4s |
...
`storage_grid_test.gd` covers `StorageGrid` — the cell arithmetic ADR 18 fixes: dimensions, the cell index round trip, the boundary rule that resolves anything outside a rack to `-1`, the 2×2×2 lattice a cell's 8 Smalls tile with no overlap, and the LIFO fill/remove pair proven as exact inverses. It was written before `StorageGrid` existed and watched to fail by naming the missing script, not by crashing. 02-05 grew it to also cover the Large-pair geometry (both orientations, ADR 25 (d)), the corner-upright mint clearance, and the occupancy DECISION rules (`cell_can_accept` / `cell_apply_record_update`) — moved into `StorageGrid` specifically so this bare `--script` run can call the real production rule rather than reimplement it a second time.

`cargo_taxonomy_test.gd` covers `CargoRecord` and `CargoCatalogue` — the goods taxonomy ADR 25 fixes: eleven categories, each with a weight band, fragility, value density and a dodgy flag, and a design-property sweep proving the weight-deception rule both ways (some categories' Smalls are already over the solo-carry limit; others are comfortably under half of it) rather than merely asserting today's specific numbers.
...
**02-06 adds four more steps (13-16), proving STORE-07 — the round-trip invariant.** Racking frees a crate's body outright (ADR 14) and retrieval mints a fresh one, so a lossy trip through a rack cell would let racking silently launder a damaged crate into a pristine one, with no error anywhere. Step 13 mutates a crate's condition and drag distance before racking it, then asserts the racked cell's stored record — and, after retrieval, the freshly minted body's own record — matches the original **field by field, naming each one**, on both peers; only `id` is deliberately excluded, since a retrieval always mints a fresh one by design (REQUIREMENTS.md's own wording: "LIFO returning a different crate of the same kind is correct"). Step 14 racks and retrieves a Medium, asserting occupancy reads 1 (not eight Small slots) and that the cell then refuses a Small outright. Step 15 racks and retrieves a masonry Medium heavy enough to exceed `Crate.SOLO_CARRY_MASS_LIMIT`, asserting the host's own mass rule (ADR 19) decides `DRAG` without being asked, that the crate rests near the deck rather than rising to eye level, and that the rack it came from does not shed a loaded neighbour in the process. Step 16 invokes the late-joiner snapshot RPC directly (this scenario is deliberately only ever two peers) and asserts the receiving peer's copy carries a real record — category and size — not merely a count.

Two things this addition found live, worth knowing before touching this file again: a **carried** Medium's own alignment torque (absent entirely from a drag) can wedge it against the rack's own corner upright while still exiting the cell it was just retrieved from — confirmed by waiting the full catch-up budget with the crate barely moving, not by reasoning about it — so a carried Medium's post-retrieval cleanup teleports it clear (`_teleport_release`) rather than walking it out on its own spring; and `_walk_to` itself now pauses in place whenever whatever it is carrying has fallen behind (`_wait_for_crate_catch_up`), because a long or awkwardly-angled walk could otherwise silently exceed `Crate.break_distance` mid-transit with no engine warning to show for it (`Crate._break_hold` prints nothing).

### goods_session

`goods_session.tscn` runs the same two-process, `--role=` shape as the other two, on its own port (**27095**), and is the first and only thing that ever calls `DayClock.begin_run()` — `carry_session.gd` and `storage_session.gd` both instance `test_room.tscn` with a dead clock, deliberately (`day_clock.gd`'s own class doc). A short day (`day_length_seconds`/`morning_seconds`, a pinned `run_seed`) is set on the instanced level's `DayClock` **before** it is added to the tree, so this scenario has a day while the other two do not.

It asserts, in order: both peers reach `MORNING` and day 1; both peers hold a manifest matching `DaySchedule.manifest_for`'s own deterministic output for the same pinned inputs (compared via `to_dict()`, not just counts); the delivery arrives — the client's own loose-crate count rises by exactly the manifest's `total_crates()`, and the newly arrived crates' records match the manifest's rows when grouped by (category, size, store_until_day), never by name, since this is the first scenario where crates are minted **during** the run rather than only at startup; every delivered crate sits inside `GoodsIn` (reusing 01-05's own zone check); the clock advances to `SHIFT` and the door reads open from `DockDoor`'s own slab position, not the clock; and in `AFTER_HOURS`, a **client's** own `request_call_it_a_night` is proven to do nothing before the **host's** own call is proven to work, both reaching `MIDNIGHT` and then day 2.

**A genuine, previously untested bug was found and fixed while writing this scenario, not merely exercised by it.** `request_call_it_a_night`'s existing `if not Net.is_host(): return` guard only checks whether the peer *currently executing* the code is the host — and because every caller (host or client) targets peer 1 via `.rpc_id(1, ...)`, that check trivially passes on the host's own machine regardless of who originally asked. Confirmed empirically with a throwaway two-process probe (never committed) before touching any code: a client's own call genuinely flipped the host to `MIDNIGHT`. Fixed by also checking `_sender_id() == 1` — the request must come from the host, not merely execute on it.

The manifest-identity check has a known limit, named rather than hidden: comparing `to_dict()` against an independently-recomputed expected value proves both peers hold a manifest matching a known, deterministic shape, but a client that (incorrectly) generated its own manifest from the same pinned inputs would pass that same check, since determinism means the two would coincidentally agree. Closing that gap is a **one-time manual step**, not permanent test code (the same shape as 02-06's own mass-field negative control): temporarily make the client generate its own manifest, confirm the assertion goes red, then revert.
